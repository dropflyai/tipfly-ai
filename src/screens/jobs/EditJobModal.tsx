...
import { updateJob, deleteJob } from '../../services/api/jobs';
...
export default function EditJobModal({
  ...
  const [jobType, setJobType] = useState<string>(job.job_type || 'restaurant');
  const [selectedColor, setSelectedColor] = useState(job.color);
  const [hourlyWage, setHourlyWage] = useState(job.hourly_wage?.toString() || '');
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      console.error('[EditJobModal] Error updating job:', error);
  ...
  const handleDelete = () => {
    Alert.alert(
      'Delete Job',
      `Are you sure you want to delete "${job.name}"? Existing tip entries will remain but won't be associated with this job.`,
      [
